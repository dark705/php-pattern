...
abstract class Handler{
	...
	public function HandleRequest($drink){//сюда можно передать как объект так и например строку. 
		if ($this->check() == $drink)		//условие возможности обработки, например у объекта можно вызвать метод
			return $this->get().static::$chain;//верни результат. static исключительно для подсчёта номера обработчика в цепотчке
		elseif ($this->_next != null){ //Если не можем и есть сл. в цепочки, передаём ему запрос и получаем от него результат
			...
			return $this->_next->HandleRequest($drink);//передаём запрос сл. в цепочке, и получаем результат от сл.
		}
		//else здесь может быть терминатор, если не один из обработчиков не обработал запрос
	}
