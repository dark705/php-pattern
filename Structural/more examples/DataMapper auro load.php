...
class Drink{
	...
	public function __construct(DrinkMapper $mapper){ 
		if ($this->angle == null)
			$mapper->load($this);// Не совсем корректно, нарушили принциы SOLID и сохранять состояние в любом случае будет Client из вне.
		//При создании объекта не что не мешает также сказать мапперу инииализировать его
	}
...
class DrinkMapper{
	...
	public function __construct(Storage $storage){
		...
		//Лучше иметь один маппер привязанный к хранилищу, но работающий с группой объектов одного класса
...
$mapper = new DrinkMapper($storage);// Маппер комплектуется хранилищем
$obj = new Drink($mapper);//объект комплектуется маппером только для начальной инициализации.
// мы не делаем $mapper->load($obj);
